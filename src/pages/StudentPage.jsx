...
function EcoMissionCard({ theme, children }) {
...
function StudentPage() {
  ...
  const [selectedEcoMission, setSelectedEcoMission] = useState(null);
  ...
  const submit = async (ecoMission) => {
    if (!selectedFile) {
      setError("Please select a photo first.");
      return;
    }
    if (!ecoMission) {
      setError("Please select an EcoMission first.");
      return;
    }
    ...
    try {
      const response = await store.submitEcoMission(ecoMission.id, selectedFile);
      ...
      if (msg.includes("functions/not-found")) {
        ...
        msg = "You have already submitted a claim for this EcoMission.";
      }
  ...
  const createQR = async () => {
    ...
    const token = await store.createRedemptionToken(100);
    if (token.error) {
  ...
  const handleNavSelect = (item) => {
    ...
    if (item === "EcoMissions") setActiveScreen("bounties");
  ...
  const captureTips = useMemo(() => {
    const tips = [];
    if (selectedEcoMission?.instructions) tips.push(selectedEcoMission.instructions);
    if (selectedEcoMission?.aiVerificationHint) tips.push(selectedEcoMission.aiVerificationHint);
          ...
          <div className="flex h-screen sm:h-[640px] flex-col overflow-hidden">
            <div className="flex shrink-0 items-center justify-between border-b border-[#edf1f3] px-4 py-3">
              <div className="flex items-center gap-0.3">
                <img src="/assets/verde-logo.png" alt="Verde Official Logo" className="h-8 w-8 shrink-0 object-contain" />
                <span className="mt-[8px] text-xl font-semibold tracking-tight text-[#008b4e]">Verde</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-[#d8f7e7] px-3 py-0.5 text-xs font-semibold text-[#087f46]">
                  {store.coinBalance} Leaves
                </span>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide">
              {activeScreen === "home" && (
                <div className="space-y-4 px-4 pt-4 pb-3">
                  ...
                  <div className="flex items-center gap-3 rounded-3xl border border-[#edf1f3] bg-[#f8fcf9] p-4 shadow-sm">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#e6f6ec] text-[24px]">🔥</div>
                    <div>
                      <div className="text-[11px] font-bold tracking-wider text-[#7c8890] uppercase leading-none mb-1">Sustainability Streak</div>
                      <div className="text-[18px] font-semibold text-[#1b1d22] leading-none mb-1">5 Days Running!</div>
                      <p className="text-[10px] text-[#008b4e]">Keep it up! You're in the top 10% this week.</p>
                    </div>
                  </div>

                  <div>
                    <div className="mb-2 flex items-center justify-between">
                      <h2 className="text-[20px] font-semibold leading-none tracking-[-0.02em] text-[#1d2025]">EcoMissions of the Day</h2>
                    </div>
                    <div className="space-y-3">
                      {allEcoMissions.slice(0, 4).map((b) => {
                        const done = store.isClaimed(b.id);
                        const ts = getThemeStyles(b.theme);
                        return (
                          <EcoMissionCard key={b.id} theme={b.theme}>
                            <div className="mb-2 flex items-center justify-between">
                            ...
                            <div className="mt-3 flex items-end justify-between">
                              <div>
                                ...
                                <div className="text-[16px] font-semibold leading-none tracking-[-0.03em] text-[#09854b]">+{b.coinReward} Leaves</div>
                              </div>
                              {done ? (
              ...
              {activeScreen === "bounties" && (
                <div className="space-y-3 px-4 pt-4 pb-3">
                  <div className="mb-2 flex items-center justify-between">
                    <h2 className="text-[20px] font-semibold leading-none tracking-[-0.02em] text-[#1d2025]">All EcoMissions</h2>
                    <span className="rounded-full bg-[#0f8f4d] px-2 py-1 text-[9px] font-semibold text-white">{ecoMissions.length} filtered</span>
                  </div>
                ...
                <div className="space-y-3 px-4 pt-4 pb-3">
                  <article className="rounded-[26px] border border-[#edf1f3] bg-white p-4">
                    <div className="mb-2 flex items-start justify-between">
                      <div>
                        <div className="mb-1 flex gap-1 text-[9px] font-semibold">
                          <span className={`rounded-full px-2 py-1 ${getThemeStyles(selectedEcoMission?.theme || "canteen").pill}`}>
                            {selectedEcoMission?.theme?.toUpperCase() || "CANTEEN"}
                          </span>
                          <span className="rounded-full bg-[#eaf0f6] px-2 py-1 text-[#60717d]">SDG {selectedEcoMission?.sdgTag || 12}</span>
                        </div>
                        <h2 className="text-[26px] font-semibold leading-[0.92] tracking-[-0.03em] text-[#1f242a]">
                          {selectedEcoMission?.title || "Bring Your Own Tumbler"}
                        </h2>
                      </div>
                    ...
                    <p className="text-[12px] leading-5 text-[#69747e]">
                      {selectedEcoMission?.description ||
                    ...
                    <button
                      type="button"
                      onClick={() => submit(selectedEcoMission || (store.ecoMissions && store.ecoMissions[0]))}
                      disabled={!selectedFile || isSubmitting}
                      className={`mt-3 w-full rounded-2xl py-3 text-[12px] font-semibold transition-colors ${selectedFile && !isSubmitting ? "bg-[#007f43] text-white" : "bg-[#dfe5e8] text-[#7b848c] cursor-not-allowed"
                        }`}
                    >
                      {isSubmitting ? (
                  ...
                  <article className="rounded-[30px] border border-[#edf1f3] bg-white p-4 text-center">
                    ...
                    {result.verdict === "approved" && (
                      <div className="mt-3 rounded-2xl bg-[#007f43] px-3 py-4 text-white">
                        <div className="text-[30px] font-semibold leading-none">+{selectedEcoMission?.coinReward || 25} Verde Coins</div>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="text-[9px] text-[#73808a]">{entry.approvedCount} EcoMissions Completed</div>
                            </div>
                          </div>
              ...
              {activeScreen === "wallet" && (
                <div className="space-y-3 px-4 pt-4 pb-3">
                  <div className="rounded-3xl border border-[#edf1f3] bg-white p-4 text-center">
                    ...
                    <div className="text-[14px] font-medium text-[#8a959d]">Total Leaves</div>
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <div className="rounded-2xl border border-[#edf1f3] bg-white px-2 py-2">
                        ...
                        <div className="text-[16px] font-semibold text-[#03884c]">{store.totalEarned} Leaves</div>
                      </div>
                      <div className="rounded-2xl border border-[#edf1f3] bg-white px-2 py-2">
                        <div className="text-[9px] tracking-[0.12em] text-[#9ba5ac]">REDEEMED</div>
                        <div className="text-[16px] font-semibold text-[#303741]">1,200</div>
                      </div>
                    </div>
                  </div>
